...
class SystemMetricsCollector(BaseCollector):
    ...
    def _collect_cpu_metrics(self, metrics):
        ...
        try:
            ...
            metrics.extend([{"name": "cpu.usage_percent",
                             "value": round(float(cpu_percent),
                                            2),
                             "type": "numeric",
            ...
            logger.warning(
                "Erreur lors de la collecte des métriques CPU : %s", exc)
    ...
    def _collect_network_metrics(self, metrics):
        ...
        try:
            ...
            metrics.extend([{"name": "network.total_bytes_sent",
            ...
            logger.warning(
                "Erreur lors de la collecte des métriques réseau : %s", exc)
        ...
        try:
            for partition in psutil.disk_partitions(all=False):
                ...
                try:
                    ...
                    metrics.extend([{"name": f"{base_name}.usage_percent",
                                     "value": round(float(disk_usage.percent),
                                                    2),
                                     "type": "numeric",
                                     "description": (f"Pourcentage d'utilisation de la partition "
                                                     f"{mountpoint} ({partition.fstype})."),
                                     "is_critical": is_critical,
                                     },
                                    {"name": f"{base_name}.total_gb",
                                     "value": round(disk_usage.total / (1024**3),
                                                    2),
                                     "type": "numeric",
                                    ...
                                    {"name": f"{base_name}.free_gb",
                                     "value": round(disk_usage.free / (1024**3),
                                                    2),
                                     "type": "numeric",
                    ...
                    logger.warning(
                        "Erreur sur la partition %s : %s", mountpoint, exc)
                    continue

        except Exception as exc:
            logger.warning(
                "Erreur lors de la collecte des métriques disque : %s", exc)
    ...
    def _collect_temperature_metrics(self, metrics):
        ...
        try:
            ...
            logger.warning(
                "Erreur lors de la collecte des températures système : %s", exc
            )
