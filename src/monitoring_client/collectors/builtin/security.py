...
def _get_ssh_port() -> int:
    ...
    try:
        ...
        logger.warning("Erreur lors de la lecture de %s : %s", ssh_config_path, exc)
...
class SecurityCollector(BaseCollector):
    ...
    def _collect_metrics(self):
        ...
        try:
            ...
            users_count = len([l for l in result.stdout.strip().splitlines() if l.strip()])
        except Exception as exc:  # pragma: no cover - log only
        ...
        try:
            for proc in psutil.process_iter(attrs=["pid", "name", "username", "cpu_percent"]):
                try:
                    ...
                    if username == "nobody" or any(kw in name for kw in suspicious_keywords):
            ...
            logger.warning(
                "Erreur lors de la récupération des processus pour la sécurité : %s",
                exc,
            )
